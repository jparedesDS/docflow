...
class AperturaView(ctk.CTkFrame):
    ...
    def _build_layout(self) -> None:
        ...
        self._field(grid, "S.REF (PO cliente)", self.var_sref, row=0, col=2, colspan=2,
                    placeholder="1000310640  ·  el año se deduce del código (P-26-… → 2026)")
        ...
        self.txt_result.insert("1.0",
            "1) Mete el código del pedido (p.ej. P-26-020) y pulsa «Localizar pedido»\n"
    ...
    def _fill_dates_from_erp(self, folder_id: str, suffix: str) -> str:
        ...
        puestas = []
        if info["order_date"]:
            self.var_f_entrada.set(info["order_date"].strftime("%Y-%m-%d"))
            puestas.append(f"entrada {info['order_date']:%d/%m/%Y}")
    ...
    def _on_create(self) -> None:
        ...
        if not pedido:
            messagebox.showerror(
                "Pedido obligatorio",
                "Introduce un código de pedido (p.ej. P-26-020).",
                parent=self,
            )
            return
