...
cálculo. La de ATLAS lo es: las que hay archivadas a mano llevan escrito
...
· **Etiquetas**, cuando la portada es una tabla de «ETIQUETA : valor» repartida
  en celdas —la de ATLAS lo es: `A2 CLIENT :` y al lado `B2 ATLAS - PLANTA SUR…`—.
...
# MISMA celda, separados por un salto de línea. Así está la de ATLAS:
#
#     A3   «Nº DOCUMENTO:\nV-2201AA00A0BG-2206-740-DL-001»
#     E3   «ITEM-TAG:\nBR10S 0001»
#
...
def _pares(raiz, tabla: list):
    ...
    · La etiqueta en una celda y el valor en la de al lado («CLIENT : | ATLAS»).
...
# sangría. La de ATLAS lo hace con FECHA: `horizontal="right" indent="8"` y
...
def _sin_sangrias_que_parten(estilos: bytes, usos: dict) -> bytes:
    ...
    for i, xf in enumerate(_XF.finditer(bloque.group(1))):
        ...
        # («A RELLENAR / POR ATLAS»); lo que se arregla es partir una palabra.
