...
def refresh_consulta_erp(dest: str | None = None, make_backup: bool = True) -> dict:
    ...
    # Escritura atómica: tmp + replace, para que el refresco horario no deje al
    # lector (Documentos/reportes) con un fichero a medio escribir.
    tmp = target.with_name(f"{target.stem}.tmp-{datetime.now():%H%M%S}{target.suffix}")
    df.to_excel(tmp, index=False, engine="openpyxl")
    import os
    os.replace(tmp, target)

    # Refrescar la caché del monitoring para que el cambio se vea sin reiniciar.
    try:
...
def auto_refresh() -> bool:
    ...
    registra y sigue (la app funciona con el consulta_erp que ya haya en disco).
    """
    try:
        if not is_configured():
            logger.info("ERP: refresco omitido (sin configuración de conexión)")
            return False
        res = refresh_consulta_erp(make_backup=False)
        logger.info("ERP: consulta_erp refrescada (%d pedidos)", res["rows"])
        return True
    except Exception as exc:  # noqa: BLE001
