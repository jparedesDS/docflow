...
ok(v["Fichero"] == "V-3005785-1100-300-TKFE2017N-CAL-001-R01.PDF", f"fichero: {v['Fichero']}")
...
def tag_de(numero, titulo="", tipo="Cálculos"):
    return P.valores_documento({"Nº Pedido": "P-26/004-S00", "Nº Doc. Cliente": numero,
                                "Título": titulo, "Tipo Doc.": tipo})["Tag"]


ok(tag_de("V-1234AB00A-1100-300-OHFE-0014-CAL-001") == "OHFE 0014",
   f"tag con guion en el número: {tag_de('V-1234AB00A-1100-300-OHFE-0014-CAL-001')!r}")
ok(tag_de("V-3005785-1100-300-TKFE2017N-CAL-001") == "TKFE 2017N",
...
ok(tag_de("V-1234AB00A-1100-300-ITP-001", "QUALITY CONTROL PLAN", "PPI") == "",
   "un ITP no es de ningún tag")
ok(tag_de("V-1234AB00A-1100-300-DOS-001", "FINAL QUALITY DOSSIER", "Dossier") == "",
   "ni el dossier")

# Y la alternativa: el tag si lo hay, «ALL TAGS» si no
calc = P.valores_documento({"Nº Pedido": "P-26/004-S00", "Título": "",
                            "Nº Doc. Cliente": "V-1234AB00A-1100-300-OUFO-0018-DWG-001"})
itp = P.valores_documento({"Nº Pedido": "P-26/004-S00", "Título": "QUALITY CONTROL PLAN",
                           "Nº Doc. Cliente": "V-1234AB00A-1100-300-ITP-001"})
...
ok(P.aplicar("3000000001-1100-3000", v) == "3000000001-1100-3000", "texto fijo intacto")
...
try:
    ...
    destino = plantilla_xlsx.rellenar(plantilla, tmp / "relleno.xlsx", marcas={
        "DOC CLIENTE": "V-3005785-1100-300-TKFE2017N-CAL-001",
    ...
    ok("V-3005785-1100-300-TKFE2017N-CAL-001" in texto, "el número del documento se escribió")
...
try:
    ...
    shared = ('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
              ...
              '<si><t>CLIENT                    :</t></si><si><t>ATLAS - PLANTA SUR</t></si>'
    ...
    ok(etiquetas["CLIENT"] == "ATLAS - PLANTA SUR", "el valor de ejemplo sale de la plantilla")
    ...
    ok(quedan.get("CLIENT") == "ATLAS - PLANTA SUR",
...
# ── Etiqueta y valor en la misma celda (la portada nueva de ATLAS) ──────────
tmp = Path(tempfile.mkdtemp())
try:
    ...
    shared = ('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
              '<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="3">'
              '<si><t>PLANTA:\nBIOS</t></si>'
              '<si><t>Nº DOCUMENTO:\nV-2201AA00A0BG-2206-740-DL-001</t></si>'
              '<si><t>ITEM-TAG:\nBR10S 0001</t></si>'
              '</sst>')
    hoja = ('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
            '<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">'
    ...
    ok(leidas["Nº DOCUMENTO"] == "V-2201AA00A0BG-2206-740-DL-001", "dos etiquetas en la misma fila")

    destino = plantilla_xlsx.rellenar(plantilla, tmp / "relleno.xlsx", valores={
        "Nº DOCUMENTO": "V-2201AA00A0BG-2206-740-CAL-001", "ITEM-TAG": "ALL TAGS"})
    quedan = {e["etiqueta"]: e["valor"] for e in plantilla_xlsx.etiquetas(destino)}
    ok(quedan["Nº DOCUMENTO"] == "V-2201AA00A0BG-2206-740-CAL-001", f"escrito: {quedan}")
...
# ── Rótulos que la sangría parte al exportar a PDF (FECHA en la de ATLAS) ────
tmp = Path(tempfile.mkdtemp())
try:
    ...
    estilos = ('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
               ...
               # 3: «A RELLENAR POR ATLAS», dos líneas entre palabras: normal
    ...
    hoja = ('<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
            ...
            '<row r="9"><c r="A9" t="inlineStr" s="3"><is><t>A RELLENAR POR ATLAS</t></is></c></row>'
